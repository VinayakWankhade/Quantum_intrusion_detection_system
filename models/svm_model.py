...
def train_svm(tune_hyperparameters=False):
    logger.info("Loading training and testing datasets for SVM...")
    ...
    if tune_hyperparameters:
        logger.info("Starting GridSearchCV for SVM hyperparameter tuning (this might take a while)...")
        param_grid = {
            'C': [0.1, 1, 10],
            'kernel': ['rbf', 'linear']
        }
        ...
        logger.info("Training SVM model with default parameters...")
        start_time = time.time()
    ...
    evaluate_model(model, X_test, y_test, model_name="SVM")
...
if __name__ == "__main__":
    train_svm(tune_hyperparameters=False)